...
func (m *milestone) ListTx(ctx context.Context, tx tx.Tx, opts ...Option) ([]*ent.Milestone, int32, int32, error) {
	...
	query := tx.Client().Milestone.Query()
...
func (m *milestone) ListIdsTx(ctx context.Context, tx tx.Tx, opts ...Option) ([]uint64, error) {
	mOpts := &MilestoneOption{}
	for _, opt := range opts {
		opt.Apply(mOpts)
	}
	...
	return query.Select(entmilestone.FieldID).IDs(ctx)
